...
#define GRANULARITY_REBUILD (1*1024*1024) // CHANGE THIS VAR to change granularity of rebuild
void rebuild_index(int slab_worker_id, struct slab *s, struct slab_callback *callback) {
   ...
   while(1) {
      ...
      if(end == start)
        ...
        int r = 0;
      start_timer {
        r = pread(fd, cached_data, end - start, start);
      } stop_add_timer(time_preads);
      if(r != end - start)
...
struct slab* create_slab(struct slab_context *ctx, int slab_worker_id, size_t item_size, struct slab_callback *callback) {
   ...
   size_t disk = get_disk_nb(); //slab_worker_id / (get_nb_workers()/get_nb_disks());
...
void read_item_async_barrier(struct slab_callback *callback) {
   callback->io_cb = read_item_async_cb;
   read_page_async_barrier(callback);
}
