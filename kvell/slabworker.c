...
static size_t get_slab_buffer(struct slab_context *ctx) {
   ...
   while(1) {
      ...
      if(pending >= ctx->max_pending_callbacks) { // Queue is full, wait
         // Comment NOP10 out for now -- don't let client wait so long
         NOP10();
         if(!PINNING)
            usleep(2);
      } else {
...
void kv_read_async_no_lookup_barrier(struct slab_callback *callback, struct slab *s, size_t slab_idx) {
   callback->slab = s;
   callback->slab_idx = slab_idx;
   return enqueue_slab_callback(s->ctx, READ_NO_LOOKUP_BARRIER, callback);
}
...
again:
   for(size_t i = 0; i < pending; i++) {
      ...
      if(action != READ_NO_LOOKUP && action != READ_NO_LOOKUP_BARRIER)
      ...
      switch(action) {
         case READ_NO_LOOKUP:
            ...
            break;
         case READ_NO_LOOKUP_BARRIER:
            read_item_async_barrier(callback);
            break;
         case READ:
...
static int emulated_cores[8] = {1,3,5,19,1,3,5,19}; //5,19};
...
static void *worker_slab_init(void *pdata) {
   ...
   uint64_t time;
   rdtscll(time);
   printf("[SLAB WORKER %lu] tid %d, time: %lu\n", ctx->worker_id, x, time);
   pin_me_on(emulated_cores[(ctx->worker_id % 4)]);
   ...
   for(size_t i = 0; i < nb_slabs; i++) {
    ...
    } stop_timer("finished create_slab");
   }
   free(cb);
   ...
   while(1) {
     ...
     start_timer {
      ...
      } stop_add_timer(worker_elapsed);
      //fprintf(stderr, "worker #%d while loop took: %lu us\n", cpu, worker_elapsed);
      ...
      while(!pending && !io_pending(ctx->io_ctx)) {
         if(!PINNING) {
            ...
            // We need to usleep instead of NOP here, so that the rocess will relinquish CPU
            //NOP10();
            usleep(1);
         }
         pending = ctx->sent_callbacks - ctx->processed_callbacks;
      ...
      //show_breakdown_periodic(1, ctx->processed_callbacks, "io_submit", "io_getevents", "io_cb", "wait", "slab_cb");
   }
...
void slab_workers_init(int _nb_disks, int nb_workers_per_disk) {
   ...
   nb_workers = nb_workers_per_disk;
