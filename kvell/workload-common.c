...
void *repopulate_db_worker(void *pdata) {
   ...
   fprintf(stderr, "about to pin repopulate db worker on: %ld\n", 20 + data->id);
   pin_me_on((20 + data->id) % 28);
...
void compute_stats(struct slab_callback *cb, void *item) {
   ...
   start_debug_timer {
      ...
      rdtscll(end);
      if (cb->scan_idx != 0) {
        uint64_t stat = cb->scan_idx;
        stat = stat << ID_OFFSET;
        stat |= (end - start);
        add_timing_stat(stat);
        if ((stat & NON_ID_MASK) != (end - start))
            fprintf(stderr, "crap, they're not equal, %lx vs %lx\n", stat & NON_ID_MASK, (end - start));
      } else
...
void* do_workload_thread(void *pdata) {
   ...
   int cpu = (20 + get_disk_nb() * d->workload->nb_load_injectors + d->id) % 28;

   init_seed();
   fprintf(stderr, "about to pin workload worker on: %d\n", cpu);
   pin_me_on(cpu);
...
void run_workload(struct workload *w, bench_t b) {
   ...
   /* If it's a scan workload AND the number of requests is small, then print scan latencies */
   if (b == ycsb_e_uniform && w->nb_requests <= 50000) {
     print_scan_latency_stats();
   }
   /* If it's a read workload, print timeseries of average read latencies */
   if (b == ycsb_c_uniform)
     print_latency_timeseries_stats();
