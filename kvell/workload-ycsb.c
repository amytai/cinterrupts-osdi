...
/* YCSB E */
static void _launch_ycsb_e(int test, int nb_requests, int zipfian, int scan_length) {
   declare_periodic_count;
   int cpu = sched_getcpu() - 9;
   ...
   for(size_t i = 0; i < nb_requests; i++) {
      /*if(random_get_put(test)) { // In this test we update with a given probability
         ...
         // Change the following line in order to change the length of scans
         tree_scan_res_t scan_res = kv_init_scan(item, scan_length); //uniform_next()%99+1); //,1000);
         free(item);
        
         size_t j = 0;
         for(; j < scan_res.nb_entries; j++) {
            struct slab_callback *cb = bench_cb();
            cb->scan_idx = (uint32_t) (cpu * nb_requests + i + 1);
         ...
         // Now do the last one, but add a barrier
         /*struct slab_callback *cb = bench_cb();
         cb->scan_idx = (uint32_t) (cpu * nb_requests + i + 1);
         cb->item = _create_unique_item_ycsb(scan_res.hashes[j]);
         kv_read_async_no_lookup_barrier(cb, scan_res.entries[j].slab, scan_res.entries[j].slab_idx);*/

         //fprintf(stderr, "submitting all of request %lu\n", i);
...
static void launch_ycsb(struct workload *w, bench_t b) {
   switch(b) {
      ...
      case ycsb_e_uniform:
         return _launch_ycsb_e(3, w->nb_requests_per_thread, 0, w->scan_length);
      case ycsb_a_zipfian:
      ...
      case ycsb_e_zipfian:
         return _launch_ycsb_e(3, w->nb_requests_per_thread, 1, w->scan_length);
      default:
